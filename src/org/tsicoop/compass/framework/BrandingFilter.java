 ...
 *
 * Static pages under web/ hardcode the default brand "TSI Compass" (and the
 * two-letter "TC" sidebar logo badge). There is no templating layer to inject
 * a runtime value, so this filter buffers each HTML response, performs a
 * literal substitution, and rewrites Content-Length to match.
 *
 * The "TC" mark badge is handled separately from BrandConfig.tokens() (rather
 * than added as a generic token) because a bare two-letter string is too easy
 * to false-positive against unrelated page text; it is only rewritten inside
 * its exact structural context, class="mark">TC<.
 *
...
public class BrandingFilter implements Filter {
    ...
    private static final String MARK_TOKEN = "class=\"mark\">TC<";
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        body = body.replace(MARK_TOKEN, "class=\"mark\">" + BrandConfig.initials() + "<");
