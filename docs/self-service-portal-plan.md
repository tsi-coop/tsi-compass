# Self-service portal for org users (raise tickets, register changes)
...
These are new methods, not modifications to `Operations.java`'s existing `listChanges`/`addChange`/`listTickets`/`addTicket` — keeps the IT-staff console's behavior untouched and isolates the new trust boundary in one place.
...
- `web/selfservice/index.html` — "My Requests": a single list combining the user's tickets and change requests with status, fed by `list_my_tickets` + `list_my_changes`.
...
### 5. IT-ops console — no changes needed

`operations-helpdesk.html` and `operations-changes.html` already read `created_by`/`requester_id` via `LEFT JOIN users` and display names (`Operations.java:103,315`), and the existing stage/status workflow (`change_requests.stage`, `helpdesk_tickets.status`) is exactly what IT staff use to triage. Self-service submissions show up there automatically — no schema or triage-UI change required.
...
7. Confirm `USER` role gets a 403 hitting `/api/v1/ops` directly (module permission `NONE`), and that `list_my_tickets`/`list_my_changes` for one `USER` never return another user's records (test with two `USER` accounts).
